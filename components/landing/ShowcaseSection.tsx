...
export function ShowcaseSection() {
  ...
  return (
    <section id="demo" className="relative z-10 px-5 py-24 sm:px-8 bg-slate-50/50">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mx-auto mt-12 overflow-hidden rounded-2xl border border-slate-200 bg-white text-left shadow-lg">
          ...
          <div className="flex items-center justify-between border-b border-slate-200 bg-slate-50 px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-2 font-mono text-xs font-semibold text-slate-700">convene-ops://demo-session</span>
            </div>
            <div className="flex items-center gap-2 font-mono text-xs text-emerald-700 font-medium">
          ...
          {/* Tab Selection Bar */}
          <div className="grid grid-cols-2 border-b border-slate-200 bg-slate-100/60 sm:grid-cols-5">
            {tabs.map((tab, i) => (
              <button
                key={tab.label}
                onClick={() => {
                ...
                className={`flex min-h-14 items-center justify-center gap-2 border-r border-slate-200 px-3 text-xs font-semibold transition-all last:border-r-0 ${
                  activeTab === i
                    ? "bg-white text-slate-900 border-b-2 border-b-emerald-600 shadow-xs"
                    : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"
                }`}
              >
                <tab.icon className={`h-4 w-4 shrink-0 ${activeTab === i ? "text-emerald-600" : "text-slate-400"}`} />
          ...
          {/* Tab Content Display */}
          <div className="min-h-[400px] p-6 sm:p-8 bg-white">
            {/* TAB 0: COPILOT AGENT (PS-3) */}
            {activeTab === 0 && (
              <div className="grid gap-6 md:grid-cols-[1.1fr_.9fr]">
                <div>
                  <p className="mb-2 text-xs font-semibold text-slate-700">Choose an example prompt to test the autonomous agent:</p>
                  <div className="mb-4 flex flex-wrap gap-2">
                    {[
                      ...
                      <button
                        key={x}
                        onClick={() => runPrompt(x)}
                        className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-700 transition-colors hover:bg-emerald-50 hover:border-emerald-300 hover:text-emerald-800"
                      >
                        {x}
                      </button>
                    ))}
                  ...
                  <div className="min-h-56 space-y-3 rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                    {!prompt && (
                      <div className="grid h-48 place-items-center text-center text-sm text-slate-500">
                        <div>
                          <Bot className="mx-auto mb-2 h-8 w-8 text-emerald-600 opacity-60" />
                    ...
                    {prompt && (
                      <div className="rounded-lg border border-slate-200 bg-white p-3 text-xs text-slate-800 shadow-xs">
                        <span className="font-semibold text-emerald-700">Your Request: </span>
                        {prompt}
                      </div>
                    )}

                    {reasoning && (
                      <div className="flex items-center gap-2 text-xs font-medium text-emerald-700 p-2">
                    ...
                    {prompt && !reasoning && (
                      <div className="space-y-2.5">
                        <div className="rounded-lg border border-emerald-100 bg-emerald-50/80 p-3 text-xs text-slate-800">
                          <span className="font-bold text-emerald-800">Convene Copilot: </span>
                        ...
                        <div className="space-y-1.5 font-mono text-xs">
                          <div className="rounded border border-emerald-200 bg-white p-2.5 text-emerald-800 shadow-xs">
                            ✓ Action: <span className="font-bold">assign_task()</span> → Assigned &ldquo;Finalize Audi 2 Venue&rdquo; to Karan Singh
                          </div>
                          <div className="rounded border border-purple-200 bg-white p-2.5 text-purple-800 shadow-xs">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                  <div className="mb-4 flex items-center justify-between text-xs text-slate-600 font-semibold">
                  ...
                  <div className="space-y-3">
                    <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-xs">
                      <div className="flex items-center justify-between text-[11px]">
                        <span className="font-semibold text-amber-700">TO DO</span>
                        <span className="text-slate-400 font-medium">HIGH PRIORITY</span>
                      </div>
                      <p className="mt-1 text-xs font-bold text-slate-900">Sponsor Pitch Deck Follow-up</p>
                    ...
                    {prompt && !reasoning ? (
                      <div className="rounded-xl border border-emerald-300 bg-emerald-50/40 p-3.5 shadow-xs">
                        ...
                        <p className="mt-1 text-xs font-bold text-slate-900">Finalize Audi 2 Venue</p>
                        <div className="mt-3 flex items-center justify-between text-xs text-slate-600">
                          <span className="font-semibold text-emerald-800">Karan Singh</span>
                          <span className="font-mono text-xs">Due Friday</span>
                        </div>
                      </div>
                    ) : (
                      <div className="rounded-xl border border-slate-200 bg-white/70 p-3.5 opacity-60">
                        <span className="text-[11px] font-medium text-slate-400">IN PROGRESS</span>
                        <p className="mt-1 text-xs text-slate-500">Venue Confirmation (Waiting for agent)</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="grid gap-4 sm:grid-cols-4">
                  {[
                    ...
                    <div
                      key={m.day}
                      className="rounded-xl border border-slate-200 bg-slate-50/70 p-4 transition-all hover:bg-white hover:shadow-xs"
                    >
                      <div
                        className={`mb-3 flex h-8 w-8 items-center justify-center rounded-full text-xs font-bold ${
                          m.done ? "bg-emerald-600 text-white" : "border-2 border-emerald-600 text-emerald-700"
                        }`}
                      >
                        {m.done ? <Check className="h-4 w-4" /> : "•"}
                      </div>
                      <div className="font-mono text-xs font-bold text-emerald-700 uppercase">{m.day}</div>
                      <h4 className="mt-1 text-sm font-bold text-slate-900">{m.title}</h4>
                      <p className="mt-1 text-xs text-slate-600">{m.desc}</p>
                      <div className="mt-4 border-t border-slate-200 pt-2 text-xs text-slate-500">
                        Lead: <strong className="text-slate-800">{m.owner}</strong>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-5">
                  <div className="mb-3 flex items-center justify-between text-xs font-semibold text-slate-600">
                    ...
                    <span className="text-emerald-700 font-mono">11:42 PM</span>
                  </div>
                  <div className="rounded-xl border border-slate-200 bg-white p-4 text-xs leading-relaxed text-slate-700 shadow-xs">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[11px] text-slate-500">{item.owner}</p>
                        </div>
                      </div>
            ...
            {activeTab === 3 && (
              <div className="space-y-4">
                <div className="rounded-xl border border-rose-200 bg-rose-50/60 p-4">
                  ...
                  <p className="ml-7.5 mt-1 text-xs text-slate-600">
                  ...
                  <div className="ml-7.5 mt-3 flex flex-wrap items-center gap-3 text-xs">
                    <span className="font-medium text-rose-700">Assigned: Karan Singh</span>
                    <span className="text-slate-300">|</span>
                    <span className="font-semibold text-emerald-700">AI Suggested Action: Draft polite reminder to University Administration</span>
                  </div>
                </div>

                <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-4">
                  <div className="flex items-center gap-2.5 font-semibold text-amber-900">
                  ...
                  <div className="ml-7.5 mt-3 flex flex-wrap items-center gap-3 text-xs">
                    <span className="font-medium text-amber-800">Current Workload: 96%</span>
                    <span className="text-slate-300">|</span>
                    <span className="font-semibold text-emerald-700">AI Recommendation: Transfer Wi-Fi setup to Rohan Verma</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {activeTab === 4 && (
              <div className="grid gap-6 md:grid-cols-[1.1fr_.9fr]">
                <div>
                  ...
                  <div className="rounded-xl border border-purple-200 bg-purple-50/50 p-4">
                    ...
                    {anchorScriptType === "intro" && (
                      <div className="rounded-lg border border-purple-200/80 bg-white p-3 text-xs leading-relaxed text-slate-800 shadow-xs">
                        &ldquo;Innovators, creators, and builders — welcome to TechFest 2026! Before we announce the hackathon challenges, please put your hands together for our keynote guest: <strong className="text-purple-800 font-bold">Dr. Ramesh Roy</strong>, Pioneer of Generative Systems!&rdquo;
                      </div>
                    )}

                    {anchorScriptType === "delay" && (
                      <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs leading-relaxed text-amber-900 shadow-xs">
                    ...
                    <div className="mt-4 flex items-center justify-between text-xs text-slate-500">
                      ...
                      <span className="font-semibold text-purple-800">Reading Time: ~30-45 seconds</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                  ...
                  <div className="space-y-2.5">
                    <div className="rounded-xl border border-emerald-200 bg-white p-3 text-xs shadow-xs">
                      <div className="flex justify-between text-[11px] font-semibold text-emerald-700">
                        <span>10:00 AM · COMPLETED</span>
                        <span>ON TIME</span>
                      </div>
                      <p className="font-bold text-slate-900 mt-1">Inauguration &amp; Welcome Address</p>
                    </div>

                    <div className="rounded-xl border border-purple-300 bg-purple-50/60 p-3 text-xs shadow-xs">
                      <div className="flex justify-between text-[11px] font-semibold text-purple-800">
                        <span>10:45 AM · LIVE ON STAGE</span>
                        <span className="text-amber-700 font-bold">+4m OVERRUN</span>
                      </div>
                      <p className="font-bold text-slate-900 mt-1">Keynote: Dr. Ramesh Roy</p>
                    </div>

                    <div className="rounded-xl border border-slate-200 bg-white p-3 text-xs opacity-80">
                      <div className="flex justify-between text-[11px] text-slate-500">
                        <span>11:34 AM (AUTOMATICALLY SHIFTED)</span>
                        <span className="text-purple-700 font-semibold">SCHEDULE UPDATED</span>
                      </div>
                      <p className="font-semibold text-slate-700 mt-1">Hackathon Challenges Announcement</p>
                    </div>
                  </div>
                </div>
              </div>
